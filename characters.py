...
class Scraper:
    ...
    def generate_csv(self, characters):
        ...
        with open("characters.csv", "w", newline='') as file:
            ...
            # Write header
            writer.writerow(["Name", "ID", "Color", "Rarity", "Tags", "Main Ability Name", "Main Ability Effect",
                             "Ultra Ability Name", "Ultra Ability Effect", "Base Stats", "Max Stats",
                             "Strike Info", "Shot Info", "Image URL", "Special Move Name", "Special Move Effect",
                             "Special Skill Name", "Special Skill Effect", "Ultimate Skill Name", "Ultimate Skill Effect",
                             "ZAbility 1 Name", "ZAbility 1 Effect", "ZAbility 2 Name", "ZAbility 2 Effect",
                             "ZAbility 3 Name", "ZAbility 3 Effect", "ZAbility 4 Name", "ZAbility 4 Effect",
                             "Is LF", "Has Zenkai"])

            for character in characters:
                row = [character.name, character.id, character.color, character.rarity,
                       ", ".join(character.tags), character.main_ability.name, character.main_ability.effect,
                       character.ultra_ability.name if character.ultra_ability else "",
                       character.ultra_ability.effect if character.ultra_ability else "",
                       character.base_stats.to_string(), character.max_stats.to_string(),
                       character.strike_info, character.shot_info, character.image_url,
                       character.special_move.name, character.special_move.effect,
                       character.special_skill.name, character.special_skill.effect,
                       character.ultimate_skill.name if character.ultimate_skill else "",
                       character.ultimate_skill.effect if character.ultimate_skill else "",
                       character.zabilities.z1.name, character.zabilities.z1.effect,
                       character.zabilities.z2.name, character.zabilities.z2.effect,
                       character.zabilities.z3.name, character.zabilities.z3.effect,
                       character.zabilities.z4.name, character.zabilities.z4.effect,
                       "Yes" if character.is_lf else "No", "Yes" if character.has_zenkai else "No"]
                writer.writerow(row)
